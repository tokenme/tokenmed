...
func AddHandler(c *gin.Context) {
	...
	db := Service.Db
	_, _, err := db.Query(`INSERT IGNORE INTO tokenme.adzones (channel_id, user_id, name) VALUES (%d, %d, '%s')`, req.ChannelId, user.Id, db.Escape(req.Name))
	if CheckErr(err, c) {
		return
	}
...
func userOwnsChannel(userId uint64, channelId uint64) bool {
	db := Service.Db
	rows, _, err := db.Query(`SELECT 1 FROM tokenme.channels WHERE user_id IN (0, %d) AND id=%d LIMIT 1`, userId, channelId)
