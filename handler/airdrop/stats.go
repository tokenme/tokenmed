...
func StatsHandler(c *gin.Context) {
	...
	rows, _, err := db.Query(`SELECT a.start_date, a.end_date, t.decimals FROM tokenme.airdrops AS a INNER JOIN tokenme.tokens AS t ON (t.address=a.token_address) WHERE a.id=%d%s`, airdropId, where)
	if CheckErr(err, c) {
		return
	}
	if len(rows) == 0 {
	...
	rows, _, err = db.Query(`SELECT SUM(pv), SUM(submissions), SUM(transactions), SUM(give_out), SUM(bonus), SUM(commission_fee), record_on FROM tokenme.promotion_stats WHERE airdrop_id=%d AND record_on>='%s' AND record_on<='%s' GROUP BY record_on ORDER BY record_on ASC`, airdropId, startDate.Format("2006-01-02"), endDate.Format("2006-01-02"))
	if CheckErr(err, c) {
		return
	}
