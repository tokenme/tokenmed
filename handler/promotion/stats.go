...
func StatsHandler(c *gin.Context) {
	...
	rows, _, err := db.Query(`SELECT SUM(ps.pv), SUM(ps.submissions), SUM(ps.transactions), SUM(ps.give_out), SUM(ps.bonus), SUM(ps.commission_fee), t.decimals, ps.record_on FROM tokenme.promotion_stats AS ps INNER JOIN tokenme.airdrops AS a ON (a.id=ps.airdrop_id) INNER JOIN tokenme.tokens AS t ON (t.address=a.token_address) WHERE ps.promotion_id=%d AND ps.record_on>='%s' AND ps.record_on<='%s' AND ps.promoter_id=%d GROUP BY ps.record_on ORDER BY ps.record_on ASC`, promotionId, startDate.Format("2006-01-02"), endDate.Format("2006-01-02"), user.Id)
	if CheckErr(err, c) {
		return
	}
